...
const int TASKS_DONE_BIT    = (1 << 2);  // Bit 2 for tasks completion
const int TASK1_LOOP_DONE_BIT = (1 << 3);  // Bit 3 for Task1 loop done
const int TASK2_LOOP_DONE_BIT = (1 << 4);  // Bit 4 for Task2 loop done
...
void Task1code( void * pvParameters )
{
  for(;;)
  {
      xEventGroupWaitBits(syncEventGroup, TASK1_STARTED_BIT | TASK2_STARTED_BIT, pdFALSE, pdTRUE, portMAX_DELAY);
      ...
      xEventGroupWaitBits(syncEventGroup, TASK2_LOOP_DONE_BIT, pdTRUE, pdFALSE, portMAX_DELAY);
...
void Task2code( void * pvParameters )
{
  for(;;)
  {
      ...
      xEventGroupWaitBits(syncEventGroup, TASK1_LOOP_DONE_BIT, pdTRUE, pdFALSE, portMAX_DELAY);
...
void loop() 
{
      ...
      // int task_Choice = Serial.parseInt();
      // Serial.println(task_Choice);

      // switch(task_Choice)
      // {
      //   case 1: runTask1 = true; break;

      //   case 2: runTask2 = true; break;

      //   default: break;
      // }

  Serial.println("");
  Serial.println("1. Start Task 1 and Task 2");
  ...
  if(task_Choice == 1)
  {
    // Set both tasks as started
    xEventGroupSetBits(syncEventGroup, TASK1_STARTED_BIT | TASK2_STARTED_BIT);

    // Wait for both tasks to complete
    xEventGroupWaitBits(syncEventGroup, TASKS_DONE_BIT, pdTRUE, pdTRUE, portMAX_DELAY);
